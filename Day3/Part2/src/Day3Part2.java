...
public class Day3Part2 {
    public static void main(String[] args) {
        ...
        int count = 0;
        try {
            myReader = new BufferedReader(new FileReader(Path.of("Day3\\Part2\\src\\Input.txt").toString()));
            carts = Arrays.stream(myReader.readLine().split(",")).mapToInt(Integer::parseInt).boxed().collect(Collectors.toSet());

            int[] cartsArray = new int[carts.size()];
            for (Integer cart : carts) {
                cartsArray[count] = cart;
                count++;
            }

            Arrays.sort(cartsArray);
